...
let fullHDMode = false; //For full HD pics
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let storedHDMode = localStorage.getItem("fullHDMode");
  fullHDMode = (storedHDMode === "true");
  if (document.getElementById("fullHDBtn")) {
    document.getElementById("fullHDBtn").textContent = fullHDMode ? "Full SD" : "Full HD";
  }
...
if (document.getElementById("fullHDBtn")) {
  document.getElementById("fullHDBtn").addEventListener("click", () => {
    fullHDMode = !fullHDMode;
    localStorage.setItem("fullHDMode", fullHDMode);
    document.getElementById("fullHDBtn").textContent = fullHDMode ? "Full SD" : "Full HD";
    
    // Update index page images:
    const indexImgs = document.querySelectorAll("#imagesGrid .image-container img");
    indexImgs.forEach(img => {
      img.src = fullHDMode ? img.dataset.original : img.dataset.display;
    });
    
    // Update favorites page images:
    const favImgs = document.querySelectorAll("#favoritesGrid .image-container img");
    favImgs.forEach(img => {
      img.src = fullHDMode ? img.dataset.original : img.dataset.display;
    });
  });
...
function createImageContainer(imgData, globalIndex) {
  ...
  img.src = fullHDMode ? imgData.original : imgData.display;
...
function groupAndRenderFavorites(favData) {
  ...
  favData.forEach((item, i) => {
    ...
    // Store both versions in dataset attributes.
    imgElem.dataset.original = item.url; 
    imgElem.dataset.display = item.url.replace("/i/", "/t/");
    // Set the source based on the global fullHDMode flag.
    imgElem.src = fullHDMode ? item.url : item.url.replace("/i/", "/t/");
